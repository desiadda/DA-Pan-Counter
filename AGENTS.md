...
- Comprehensive Reference Resolution: When refactoring or relocating logic (e.g. moving settings tabs, deleting pages), immediately scan all other components and files for duplicate UI segments, stale callback handlers, or legacy imports related to the change. Resolve them immediately in the same step to prevent runtime crashes.
- **i18n Rule (Mandatory — all new/updated UI)**: All visible text MUST go through the translation system: `useT(lang)` / `t(key, lang)` from `src/lang/translations.ts` with keys in `translations` (en, hi, th, my). Never hardcode visible strings — especially NEVER mixed-language/Hinglish text. English mode must be pure English, Hindi mode pure Hindi, etc. When adding or editing a feature: (1) add translation keys for ALL four languages in `src/lang/translations.ts`, (2) render through `tr("section.key")` — no raw strings in JSX or `alert()`/`confirm()`/placeholder/`title` attributes, (3) `t()` falls back to English for missing keys — do not rely on that; all four languages must be filled, (4) existing `lang === "hi" ? ... : ...` ternaries should be migrated to `t()` when touched, (5) domain words (Udhaar, Khata, COH, PO) may stay same in English but must be translated in hi/th/my.
- **Strict Pre-Execution Thumb Rule Verification (Mandatory)**: Kuchh bhi naye update, feature, refactor ya bug fix par kaam karne se pehle, sabhi thumb rules aur project guidelines (i18n, Zustand state, Firestore sync, React Router, etc.) ko dhyan se review karke aur poori tarah follow karke hi aage ka kaam honga.
