    ...
    <div style={styles.overlay} onClick={onClose}>
      <div style={styles.panel} onClick={e => e.stopPropagation()}>
        ...
        <div style={styles.topSection}>
          ...
          {pending.length > 0 && (
            <div style={styles.pendingNotificationBanner}>
              <div style={{ fontWeight: 700, color: "#b45309", marginBottom: "6px", fontSize: "0.85rem", display: "flex", alignItems: "center", gap: "6px" }}>
                <span>📩</span> <span>{tr("coh.pendingReceived")}</span>
              </div>
              {pending.map(tx => (
                <div key={tx.id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", background: "var(--bg, #ffffff)", padding: "8px 10px", borderRadius: "8px", border: "1px solid #fde68a", marginTop: "4px" }}>
                  <div>
                    <div style={{ fontWeight: 700, fontSize: "0.85rem" }}>From: {tx.fromUserName}</div>
                    <div style={{ fontSize: "1rem", fontWeight: 800, color: "#047857" }}>฿{(tx.amount || 0).toFixed(2)}</div>
                    {tx.note && <div style={{ fontSize: "0.7rem", color: "var(--text-muted)" }}>{tx.note}</div>}
                  </div>
                  <div style={{ display: "flex", flexDirection: "row", gap: "6px" }}>
                    <button onClick={() => handleApprove(tx.id)} disabled={submitting} className="btn btn-primary" style={{ padding: "0.3rem 0.6rem", fontSize: "0.75rem", whiteSpace: "nowrap" }}>
                      {submitting ? "..." : "✓ Approve"}
                    </button>
                    <button onClick={() => handleReject(tx.id)} disabled={submitting} className="btn btn-danger" style={{ padding: "0.3rem 0.6rem", fontSize: "0.75rem", whiteSpace: "nowrap" }}>
                      ✕ Reject
                    </button>
                  </div>
                </div>
          ...
          <div style={styles.tabs}>
            {[
              { key: "balance", label: `💰 ${tr("coh.balance")}` },
              ...
              { key: "statement", label: `📊 ${tr("coh.statement")}` },
            ...
            <div style={styles.section}>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: "0.75rem", marginBottom: "1rem" }}>
                <div style={{ display: "flex", justifyContent: "space-between", fontSize: "0.85rem", color: "var(--text-muted)" }}>
                  <span>{tr("coh.systemBalance")}</span>
                    ...
                    <div style={{ 
                      ...
                      <div style={{ fontSize: "0.75rem", color: "#64748b" }}>
                        {diff === 0 
                          ? tr("coh.perfectMatch") 
                          : diff < 0 
                            ? `${tr("coh.shortageMsg")} (฿${absDiff.toFixed(2)})`
                            : `${tr("coh.surplusMsg")} (฿${absDiff.toFixed(2)})`
                        }
                      </div>
                    </div>
                  );
                ...
                >
                  {submitting ? tr("common.loading") : tr("coh.confirmReconcile")}
                </button>
              </div>
            </div>
          )}
...
const styles = {
  overlay: {
    position: "fixed", inset: 0, backgroundColor: "var(--overlay, rgba(0,0,0,0.5))",
    backdropFilter: "blur(4px)",
    display: "flex", alignItems: "flex-end", justifyContent: "center",
    zIndex: 1000,
  },
  panel: {
    backgroundColor: "var(--card-bg, #ffffff)", color: "var(--text, #1e293b)",
  ...
  pendingNotificationBanner: {
    background: "var(--card-bg, #fffbebf5)", border: "1.5px solid #f59e0b",
    borderRadius: "12px", padding: "10px", maxHeight: "150px", overflowY: "auto",
  },
  tabs: {
  ...
  tab: {
    ...
    color: "var(--text-muted, #64748b)", background: "var(--hover-bg, #f1f5f9)",
    border: "1px solid var(--border, #cbd5e1)", borderRadius: "10px",
    cursor: "pointer", whiteSpace: "nowrap", fontFamily: "inherit",
    transition: "all 0.15s ease",
  },
  activeTab: {
    backgroundColor: "var(--primary, #047857)", color: "#ffffff",
